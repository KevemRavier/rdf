...
let souHost = false; // NOVO: Identifica se é o criador da sala
...
function escolherModo(modo) {
    ...
    if (modo === 'single') {
        ...
        // Mostrar botões de nível no modo solo
...
btnCriarSala.addEventListener('click', () => {
    ...
    peer.on('connection', (conn) => {
        ...
        // Host vai para seleção de nível
        setTimeout(() => {
            document.getElementById('tela-multiplayer').style.display = 'none';
            document.getElementById('tela-nivel').style.display = 'block';
            // Mostrar botões de nível para o host
            document.getElementById('botoes-nivel').style.display = 'block';
        }, 1000);
    });
...
btnConectar.addEventListener('click', () => {
    ...
    peer.on('open', () => {
        ...
        connection.on('open', () => {
            ...
            // Guest vai para tela de espera
            setTimeout(() => {
                document.getElementById('tela-multiplayer').style.display = 'none';
                document.getElementById('tela-nivel').style.display = 'block';
                // Esconder botões de nível para o guest
...
function configurarConexao() {
    connection.on('data', (data) => {
        if (data.type === 'score') {
            ...
            // Guest recebe o nível escolhido pelo host
...
// CORREÇÃO: Modal de fim de partida funcional
function mostrarModalFimPartida() {
    const modal = document.getElementById('modal-fim-partida');

    // Atualizar placar no modal
    document.getElementById('modal-my-wins').textContent = minhasVitorias;
    document.getElementById('modal-opponent-wins').textContent = vitoriasOponente;

    // Definir mensagem
    const modalMensagem = document.getElementById('modal-mensagem');
    if (minhasVitorias > vitoriasOponente) {
        modalMensagem.textContent = '🎉 Você está na frente!';
        modalMensagem.style.color = '#48bb78';
    } else if (vitoriasOponente > minhasVitorias) {
        modalMensagem.textContent = '💪 O oponente está na frente!';
        modalMensagem.style.color = '#f56565';
    } else {
        modalMensagem.textContent = '🤝 Empate!';
        modalMensagem.style.color = '#ed8936';
    }
...
function configurarBotoesModal() {
    ...
    const btnNovaPartidaNovo = btnNovaPartida.cloneNode(true);
    const btnVoltarMenuNovo = btnVoltarMenu.cloneNode(true);

    btnNovaPartida.parentNode.replaceChild(btnNovaPartidaNovo, btnNovaPartida);
    btnVoltarMenu.parentNode.replaceChild(btnVoltarMenuNovo, btnVoltarMenu);

    // Adicionar novos listeners
    btnNovaPartidaNovo.addEventListener('click', () => {
        if (typeof $ !== 'undefined') {
    ...
    btnVoltarMenuNovo.addEventListener('click', () => {
        if (typeof $ !== 'undefined') {
            $('#modal-fim-partida').modal('hide');
        } else {
            document.getElementById('modal-fim-partida').style.display = 'none';
...
function reiniciarPartida() {
    ...
    // Voltar para seleção de nível (host escolhe, guest espera)
    document.getElementById('tela-jogo').style.display = 'none';
    document.getElementById('tela-nivel').style.display = 'block';

    if (modoMultiplayer) {
        if (souHost) {
            // Host pode escolher nível
            document.getElementById('botoes-nivel').style.display = 'block';
        } else {
            // Guest espera
            document.getElementById('botoes-nivel').style.display = 'none';
...
function voltarParaSelecaoNivel() {
    ...
    // Resetar vitórias ao voltar para seleção
    minhasVitorias = 0;
    vitoriasOponente = 0;
}
...
function selecionarNivel(nivel, enviarParaOponente = true) {
    ...
    if (modoMultiplayer) {
        ...
        // Adicionar display de vitórias no placar
...
function iniciar() {
    ...
    // Não inicia o timer automaticamente no multiplayer
    if (!modoMultiplayer) {
        iniciarTimer();
    }
...
inputPalavra.addEventListener('input', () => {
    if (inputPalavra.value === palavraOriginal) {
        ...
        // Se for a primeira palavra no multiplayer, inicia o jogo
